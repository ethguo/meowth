...
Adafruit_NeoPixel fire = Adafruit_NeoPixel(FNUMPIXELS, FPIN, NEO_GRB + NEO_KHZ800);
...
//Now begins the main loop. 
  ...
  //Get serial data for Neopixels
  if (Serial.available() > 0) {
    // read the incoming byte:
    Serial.readBytes(pixel, 4);
   }
  //Load buffer for NeoPixels
   if (pixel[0] < NUMPIXELS) {
    strip.setPixelColor(pixel[0], strip.Color(pixel[1], pixel[2], pixel[3]));
   }
   else if (pixel[0] < FNUMPIXELS) {
    fire.setPixelColor(pixel[0], fire.Color(pixel[1], pixel[2], pixel[3]));
   }
  ...
  if(x<250) {
    ...
    for(int i=0;i<NUMPIXELS;i++){
      strip.setPixelColor(i, strip.Color(20*i,0,0));
      strip.show();
      delay(10);
    }
    delay(100);
